...
fn workload(query: &str, accuracy: AccuracyTarget) -> QueryWorkload {
    QueryWorkload {
...
pub fn lower_promql(query: &str, accuracy: AccuracyTarget) -> Result<QueryExpr, PromqlError> {
    let mut lowered = lower_promql_workload(&workload(query, accuracy))?;
    Ok(lowered.remove(0))
}
...
pub fn lower_promql_with_histograms(
    ...
    let mut lowered =
        lower_promql_workload_with_histograms(&workload(query, accuracy), histograms)?;
    Ok(lowered.remove(0))
}
