...
fn try_promote_additive_top_ranking(expr: &QueryExpr) -> Option<QueryExpr> {
    ...
    // derived child such as Rate/Increase is different: a heavy-hitter sketch
    // may propose candidate membership, but PromQL still requires exact
    // reset-aware/extrapolated values to rerank those candidates. The current
