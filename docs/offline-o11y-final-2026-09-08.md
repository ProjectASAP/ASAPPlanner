...
See the [final sweep report](../tools/empirical-bench/results-sweep/MEASUREMENTS.md).
...
available in the [exact snapshot experiment](../tools/empirical-bench/results/o11y-exact-snapshot.json).
...
require local Cargo source patches. Source code, tools, and results are delivered
through coordinated PRs in the two repositories, without automatically merging
them or closing the issue.
