...
datetime constant-evaluation helpers. The parser remains pinned to that
third-party source plus the compatibility fixes described below. Vendoring both path crates
...
The small parser compatibility patch set preserves official MetricsQL behavior
for OR matcher branches, `WITH` string interpolation, and numeric separators;
it is checked against canonical output generated by the official
VictoriaMetrics Go parser v0.84.4.
...
fails closed. The pinned Rust parser also has 21 known failing library tests and
3 known failing doctests inherited from its upstream baseline. CI runs all of
them and checks the explicit failure-name allowlist, so a new failure cannot be
hidden by excluding the vendored crate from the workspace.
